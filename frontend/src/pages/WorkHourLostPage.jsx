...
import WorkHourLostCharts from '../components/WorkHourLostCharts'
...
export default function WorkHourLostPage() {
  const [active, setActive] = useState('function')
  const current = tabs.find(t => t.key === active)
  const baseKey = current?.base || 'function'
  const { data = [], isLoading, isError, error } = useQuery({ queryKey: ['work_hour_lost', baseKey], queryFn: () => getWorkHourLost(baseKey), retry: 0 })
  ...
  const cols = [
    ...
    { key: 'lost', label: 'Work Hours Lost' },
    { key: 'lost_pct', label: 'Work Hour Lost %' },
  ]
    ...
    <div className="space-y-4">
      ...
      {current?.mode === 'chart' && !isLoading && !isError && (
        <div className="space-y-3">
          ...
          <WorkHourLostCharts rows={filtered} />
        </div>
      )}
    </div>
  )
