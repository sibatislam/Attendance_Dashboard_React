...
export default function LeaveAnalysisCharts({ rows }) {
  ...
  const SLPercentLabel = (props) => {
    const { x, y, value } = props
    if (x == null || y == null) return null
    return <text x={x} y={y - 8} fill="#f59e0b" fontSize={12} fontWeight="700" textAnchor="middle" dominantBaseline="bottom">{value}%</text>
  }

  const CLPercentLabel = (props) => {
    const { x, y, value } = props
    if (x == null || y == null) return null
    return <text x={x} y={y - 8} fill="#ef4444" fontSize={12} fontWeight="700" textAnchor="middle" dominantBaseline="bottom">{value}%</text>
  }

  const APercentLabel = (props) => {
    const { x, y, value } = props
    if (x == null || y == null) return null
          ...
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <ComposedChart data={byGroup.get(g)} margin={{ top: 30, right: 20, bottom: 0, left: 0 }}>
                ...
                <YAxis yAxisId="right" orientation="right" label={{ value: 'SL%, CL% & A%', angle: -90, position: 'insideRight' }} domain={[0, 100]} />
                <Tooltip />
                <Legend />
                ...
                <Line yAxisId="right" type="monotone" dataKey="sl_pct" name="SL %" stroke="#f59e0b" strokeWidth={2} dot>
                  <LabelList content={<SLPercentLabel />} />
                </Line>
                <Line yAxisId="right" type="monotone" dataKey="cl_pct" name="CL %" stroke="#ef4444" strokeWidth={2} dot>
                  <LabelList content={<CLPercentLabel />} />
                </Line>
                <Line yAxisId="right" type="monotone" dataKey="a_pct" name="A %" stroke="#8b5cf6" strokeWidth={2} dot>
