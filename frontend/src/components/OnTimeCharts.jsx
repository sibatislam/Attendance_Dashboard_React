...
export default function OnTimeCharts({ rows }) {
  // rows: [{ month: '2025-07', group: 'MBD', members, on_time_pct }]
  const byGroup = useMemo(() => {
    ...
    // sort each series by month label order using YYYY-MM from month
          ...
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <ComposedChart data={byGroup.get(g)} margin={{ top: 30, right: 20, bottom: 0, left: 0 }}>
                ...
                <YAxis yAxisId="right" orientation="right" label={{ value: 'On Time %', angle: -90, position: 'insideRight' }} domain={[0, 100]} />
                <Tooltip />
                <Legend />
                ...
                <Line yAxisId="right" type="monotone" dataKey="on_time_pct" name="On Time %" stroke="#f59e0b" strokeWidth={2} dot>
